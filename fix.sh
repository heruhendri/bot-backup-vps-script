...
# ======================================================
# AUTO BACKUP VPS — INSTALLER HEADER
# ======================================================
...
IFS=',' read -r -a FOLDERS <<< "\$FOLDERS_RAW"
for f in "\${FOLDERS[@]}"; do
    if [ -d "\$f" ]; then cp -a "\$f" "\$TMP_DIR/" || true; fi
done
...
EOT
    ...
    systemctl daemon-reload || true
    systemctl enable --now auto-backup.timer || true
    echo "[OK] Installer files rebuilt."
}
...
while true; do
    ...
    echo "15) Rebuild / Repair installer files"
